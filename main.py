...
def get_all_budget_stats(user_id):
    """Budget usage figures for every period (daily/weekly/monthly/yearly) at once.
    Used by the Reports page so all four can be shown together in one glance."""
    return {bt: get_budget_stats(user_id, bt) for bt in BUDGET_TYPES}
...
(function(){
  ...
  var pref = root.getAttribute('data-theme') || 'blue';
...
html[data-theme="dark"]{
  --bg-deep:#000000; --bg-mid:#0a0a0a; --bg-grad-end:#141414;
  --accent-1:#3b82f6; --accent-2:#22d3ee; --accent-3:#6366f1;
...
html[data-theme="dark"] .bottom-nav{background:rgba(0,0,0,.85);}
...
""" + APP_SHELL_HEAD + """
  ...
  <div class="glass card-app">
    ...
    <div class="row g-2">
      {% for bt in ['daily','weekly','monthly','yearly'] %}
        {% set b = budget_overview[bt] %}
        <div class="col-6">
          <div class="glass card-app mb-0" style="padding:12px;">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <span class="fw-semibold" style="font-size:.8rem;">{{ bt|capitalize }} Usage</span>
              {% if b.budget_amount > 0 %}
              <span class="pill {{ 'pill-danger' if b.budget_pct >= 100 else ('pill-warning' if b.budget_pct >= 80 else 'pill-success') }}" style="font-size:.66rem;">{{ b.budget_pct }}%</span>
              {% endif %}
            </div>
            {% if b.budget_amount > 0 %}
              <div class="progress mb-2" style="height:6px;"><div class="progress-bar" style="width:{{ [b.budget_pct, 100]|min }}%; {{ 'background:linear-gradient(90deg,#f43f5e,#f97316);' if b.budget_pct>=100 else ('background:linear-gradient(90deg,#f59e0b,#fbbf24);' if b.budget_pct>=80 else '') }}"></div></div>
              <div class="text-lo" style="font-size:.7rem;">Spent: ₱{{ '%.2f'|format(b.period_expense) }}</div>
              <div class="text-lo" style="font-size:.7rem;">Left: ₱{{ '%.2f'|format(b.budget_remaining) }}</div>
            {% else %}
              <div class="text-lo" style="font-size:.7rem;">Spent: ₱{{ '%.2f'|format(b.period_expense) }}</div>
              <div class="text-lo mt-1" style="font-size:.66rem;"><i class="bi bi-info-circle"></i> No budget set</div>
            {% endif %}
          </div>
        </div>
      {% endfor %}
    </div>
    <a href="{{ url_for('budget') }}" class="btn-ghost w-100 py-2 text-center mt-2" style="font-size:.8rem;"><i class="bi bi-sliders"></i> Manage Budgets</a>
  </div>
...
""" + APP_SHELL_HEAD + """
  ...
  <div class="glass card-app">
    ...
    <div class="row g-2" id="themeSelector">
      ...
      <div class="col-3">
        <button type="button" onclick="chooseTheme('{{ th }}', this)" class="btn-ghost w-100 py-3" data-theme-btn="{{ th }}" data-no-spin="1" style="{{ 'border-color:var(--accent-2); box-shadow:0 0 0 2px rgba(34,211,238,.3);' if user.theme==th }}">
...
{% block extra_scripts %}
<script>
function chooseTheme(theme, btn){
  var root = document.documentElement;
  ...
  if(btn){
    ...
    btn.style.boxShadow = '0 0 0 2px rgba(34,211,238,.3)';
  }
  var fd = new FormData();
  ...
  fetch('{{ url_for("update_theme") }}', {
    ...
    .then(function(){ showToast('Theme updated.', 'success'); })
    .catch(function(){ showToast('Could not save theme. It will still apply for this session.', 'warning'); });
}
</script>
{% endblock %}
"""
...
def reports():
    ...
    budget_overview = get_all_budget_stats(session["user_id"])
    return render_template("reports.html", cd=cd, budget_overview=budget_overview,
                            year=datetime.now().year, active="reports")
